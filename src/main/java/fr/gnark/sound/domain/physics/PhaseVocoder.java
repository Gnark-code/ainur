...
public class PhaseVocoder {
    ...
    public double[] proceed(final double[] data) {
        ...
        while (iterator.hasNext()) {
            ...
            //  /
            final Complex[] newSignal = processing2(analysis, previousFramePhase, previousDeltaPhis, phaseOut);
    ...
    private double[] getWindowedOutputFrame(final Complex[] newSignal) {
        ...
        for (int n = 0; n < newSignal.length; n++) {
            outputFrame[n] = (newSignal[n].getReal() * w[n]) / Math.sqrt(windowSize / (2 * hopSynthesis));
        }
        return outputFrame;
    }
     ...
     /*           final double deltaPhi  = phases[index]  - previousFramePhase[index];
        ...
        */
          ...
          /*      final double deltaPhi  = phases[index]  - previousFramePhase[index];
            ...
            newSignal[index] = new Complex(magnitudes[index]).multiply(Complex.I.multiply(phaseOut[index]).exp()); //new Complex(magnitudes[index]).multiply(Complex.I.multiply((omegaBin * (index / sampleFrequency)) + phaseOut[index]).exp());
    ...
    private Complex[] processing2(final FftResult analysis, final double[] previousFramePhase, final double[] previousDeltaPhis, final double[] phaseOut) {
        final double[] magnitudes = analysis.getMagnitudes();
        final double[] phases = analysis.getPhases();
        final double ratio = hopSynthesis / hopAnalysis;
        ...
        while (index < magnitudes.length) {
            ...
            double deltaPhi = omegaBin + unwrapPhase3(phases[index] - previousPhaseIn - omegaBin);
            phaseOut[index] = unwrapPhase3(previousPhaseOut + deltaPhi * ratio);
            //newSignal[index] = new Complex(magnitudes[index] * Math.cos(phaseOut[index]), magnitudes[index] * Math.sin(phaseOut[index]));
            //newSignal[index] = analysis.analysis[index];
            final double magnitude = magnitudes[index];
    ...
    private double unwrapPhase3(final double phaseIn) {
